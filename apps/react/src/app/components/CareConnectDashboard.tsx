...
export function CareConnectDashboard() {
  ...
  return (
    <div id="main-content" className="min-h-screen bg-transparent pb-20 lg:pb-0">
      <div className="p-4 md:p-6 lg:p-8">
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
            ...
            {/* Quick-action buttons — open their respective modals */}
            <div className="flex flex-wrap gap-2 lg:gap-3">
              <button
                type="button"
                onClick={() => setTaskModalOpen(true)}
                className="flex items-center gap-2 px-4 h-10 md:h-12 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold rounded-lg shadow-lg transition-all"
              >
                <Plus className="w-4 h-4 md:w-5 md:h-5" strokeWidth={2.5} aria-hidden="true" />
                <span className="hidden sm:inline">New Task</span>
              </button>
              <button
                type="button"
                onClick={() => setAppointmentModalOpen(true)}
                className="flex items-center gap-2 px-4 h-10 md:h-12 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border-2 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-semibold rounded-lg shadow-sm transition-all"
              >
                <Calendar className="w-4 h-4 md:w-5 md:h-5" strokeWidth={2} aria-hidden="true" />
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 h-10 md:h-12 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border-2 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-semibold rounded-lg shadow-sm transition-all"
              >
                <UserPlus className="w-4 h-4 md:w-5 md:h-5" strokeWidth={2} aria-hidden="true" />
                <span className="hidden sm:inline">New Patient</span>
              </button>
            </div>
          </div>

          {/* Global search bar — ⌘K shortcut hint shown on the right */}
          <div className="relative">
            ...
            />
            <kbd className="absolute right-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-xs font-mono rounded border border-slate-300 dark:border-slate-600">
              ⌘K
            </kbd>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
          {summaryCards.map((card) => {
            ...
            return (
              <div
                key={card.title}
                className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 md:p-6 hover:shadow-lg transition-shadow"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className={`w-10 h-10 md:w-12 md:h-12 ${card.bgColor} rounded-xl flex items-center justify-center`}>
                    <Icon className={`w-5 h-5 md:w-6 md:h-6 ${card.textColor}`} strokeWidth={2} aria-hidden="true" />
                  </div>
                </div>
        ...
        {/* Main Content Grid — Urgent Tasks | Today's Schedule | Recent Activity */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <li key={task.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      <Users className="w-3 h-3" aria-hidden="true" />{task.patient}
                    </span>
                    <span className="flex items-center gap-1">
          ...
          <div className="lg:col-span-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30">
              <div className="flex items-center gap-2">
                <CalendarDays className="w-5 h-5 text-green-600 dark:text-green-400" strokeWidth={2} aria-hidden="true" />
          ...
          <div className="lg:col-span-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-950/30 dark:to-blue-950/30">
              <div className="flex items-center gap-2">
                <Activity className="w-5 h-5 text-cyan-600 dark:text-cyan-400" strokeWidth={2} aria-hidden="true" />
                  ...
                  <li key={activity.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="w-8 h-8 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center">
                        <Icon className="w-4 h-4 text-slate-600 dark:text-slate-300" strokeWidth={2} aria-hidden="true" />
                      </div>
                      <div className="flex-1">
      ...
      {/* Modals — rendered outside the main content flow to avoid stacking context issues */}
      <NewAppointmentModal
