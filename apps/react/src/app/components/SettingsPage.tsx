...
export function SettingsPage() {
  ...
  // Initialize local state from the global context settings
  ...
  const handleSave = () => {
    // Save to LocalStorage and Global State via Context
  ...
  const handleCancel = () => {
    setSettings(state.settings); // Revert to global state
    setHasUnsavedChanges(false);
  };
              ...
              <div className="flex min-w-max md:min-w-0">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`flex-1 px-6 py-4 text-sm md:text-base font-semibold transition-all focus:outline-none ${activeTab === tab.id ? 'text-blue-600 border-b-2 border-blue-600 bg-white' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
                  >
                    {tab.label}
                  </button>
                ))}
            ...
            <div className="p-6 md:p-8 min-h-[400px]">
              {activeTab === 'general' && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <section>
                    {sectionHeader('Layout Preferences')}
                    <div className="bg-slate-50 rounded-lg px-4 divide-y divide-slate-200 border border-slate-200">
                      <ToggleSwitch
                        enabled={settings.leftHandedMode}
                        onChange={(val) => updateSetting('leftHandedMode', val)}
                        label="Left-Handed Mode"
                        description="Optimize layout for left-handed users (moves sidebar to the right)"
                      />
                    </div>
                  </section>
              ...
              {activeTab === 'accessibility' && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <section>
                    {sectionHeader('Keyboard & Navigation')}
                    <div className="bg-slate-50 rounded-lg px-4 divide-y divide-slate-200 border border-slate-200">
                      <ToggleSwitch enabled={settings.enhancedKeyboardNav} onChange={(val) => updateSetting('enhancedKeyboardNav', val)} label="Enhanced Keyboard Navigation" description="Enable advanced keyboard shortcuts" />
                      <ToggleSwitch enabled={settings.alwaysFocusIndicators} onChange={(val) => updateSetting('alwaysFocusIndicators', val)} label="Always Show Focus Indicators" description="Display focus rings at all times" />
                    </div>
                  </section>
                  <section>
                    {sectionHeader('Visual Accessibility')}
                    <div className="bg-slate-50 rounded-lg px-4 divide-y divide-slate-200 border border-slate-200">
                      <ToggleSwitch enabled={settings.highContrastMode} onChange={(val) => updateSetting('highContrastMode', val)} label="High Contrast Mode" description="Increase color contrast for readability" />
                      <ToggleSwitch enabled={settings.reduceMotion} onChange={(val) => updateSetting('reduceMotion', val)} label="Reduce Motion" description="Minimize animations for reduced sensitivity" />
                    </div>
                  </section>
                </div>
              )}
              ...
              {activeTab === 'notifications' && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <section>
                    ...
                    <div className="bg-slate-50 rounded-lg px-4 divide-y divide-slate-200 border border-slate-200">
                      <ToggleSwitch enabled={settings.taskReminders} onChange={(val) => updateSetting('taskReminders', val)} label="Task Reminders" description="Receive notifications for upcoming tasks" />
                      <ToggleSwitch enabled={settings.urgentTaskAlerts} onChange={(val) => updateSetting('urgentTaskAlerts', val)} label="Urgent Task Alerts" description="Get priority notifications" />
                    </div>
                  </section>
                  <section>
                    {sectionHeader('Reminder Settings')}
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 sm:justify-between items-center">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  <button 
                    ...
                    className="flex-1 sm:flex-none h-12 px-6 bg-white hover:bg-slate-100 text-slate-700 font-semibold rounded-lg border-2 border-slate-300 transition-all disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={handleSave} 
                    disabled={!hasUnsavedChanges}
                    className={`flex-1 sm:flex-none h-12 px-6 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold rounded-lg shadow-lg transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${hasUnsavedChanges ? '' : 'opacity-50 cursor-not-allowed'}`}
                  >
                    Save Changes
                  </button>
                </div>
